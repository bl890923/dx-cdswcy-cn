...
function showProductTips(oid) {
    ...
    if (jLayer.length == 0) {
        var sHtml = "<div class='mesbook5' id='div_nsw_tips'><h1><a href='javascript:void(0)' onclick=\"$(this).parent().parent().fadeOut(80);hideFullBg('div_nsw_tips_bg')\"><img src='images/ico9_close.gif' /></a>产品预定</h1>"
	            +"<h4>告诉我该产品的好消息</h4>"
   ...
   $(function () {
       ...
       */
       ...
       for (var i = count - 1; i >= 0; i--) {
           ...
           //主导航的匹配链接
           var $a = $(".navMain a[href*='" + current + "']");

           //主导航的匹配文字  [一般情况下是没有用的,有时候会碰到说是这个文字,但是链接确是另外一个,但是导航选中还是要加在文字上面]
           //比如 二级分类是成功案例,但是它下面还有一个三级分类是经典案例,主导航上的是二级分类确标着三级分类的链接,这个文字就有用了
           //况且这里还要先判断文字再判断链接
           var $t = $(".navMain a:contains('" + txt + "')");
       ...
       if (!$(".navMain li.cur").size()) {
           $(".navMain a[href*='" + MARK + "']").parent("li").addClass("cur").siblings("li").removeClass("cur");
           //如果还没有主导航选中,给第一个加样式
           if (!$(".navMain li.cur").size()) {
               $(".navMain li:first").addClass("cur").siblings("li").removeClass("cur");
           }
       } else {
       }
